...
Это руководство по созданию среды разработки для микроконтроллеров семейства STM32 в дистрибутивах GNU Linux с использованием кодогенератора от ST CubeMX и OpenOCD в качестве ПО для загрузчика и отладчика. Используемый дистрибутив - Manjaro Linux версии 23.1.1, отладочная плата STM32F411.

## Установка необходимых пакетов для создания среды разработки
Для установки можно пользоваться пакетным менеджером Manjaro `pamac` или пакетным менеджером Arch `pacman`.

Понадобиться установить менеджер пакетов yay:
```
pamac install yay
```
...
yay -S stm32cubemx
```
Eсли возникли проблемы с установкой yay, то можно воспользоваться AUR :
- Склонировать репозиторий CubeMX: 
    ```
    git clone https://aur.archlinux.org/stm32cubemx.git
    ```
- Перейти в директорию `stm32cubemx` и для установки выполнить:
    ```
    makepkg -si PKGBUILD
    ```
...
### openOCD
В качестве инструмента для отладки и загрузки прошивки используется openOCD, для загрузки выполнить:
```
pamac install openocd
```
Также понадобится *.svd файл для просмотра регистров во время отладки. Этот файл можно взять с сайта [ST](https://www.st.com/en/microcontrollers-microprocessors/stm32-32-bit-arm-cortex-mcus.html)
...
В скачанном архиве находится директория *STM32F4_svd_V**, где * версия релиза. Сохраняете ее в удобном месте.

## Создание проекта в CubeMX
Самый простой способ получить заготовку проекта - собрать его в CubeMX. Сконфигурируем проект. Добавим пин отладочного интерфейса для мигания светодиодом. Далее необходимо выполнить несколько обязательных пунктов:
- Во вкладке **Project** 
    ...
    Ссылка на репозиторий, если вам хочется узнать больше информации:
        ...
        Если выбрали свое расположение хранения, то не забудьте снять галочку с *Use Default Firmware Location* и указать расположение прошивки.
...
- Для загрузки прошивки на микроконтроллер, представленной как *filename.elf* необходимо выполнить команду:
    ```
    openocd -f board/stm32f4discovery.cfg -c "program filename.elf verify reset exit"
    ```

- Для загрузки прошивки на микроконтроллер, представленной как *filename.hex* необходимо выполнить команду:
    ```
    openocd -f board/stm32f4discovery.cfg -c "program filename.hex verify reset exit"
    ```

- Для загрузки прошивки на микроконтроллер, представленной как *filename.bin* требуется передать адрес начала flash памяти. Выполнить команду:
    ```
